			...
			for (int j = 0.; j < num_swarms; j++)
			{
				if (k == 0)
				{
					...
					*err_best_s = MaxValue;
					...
					HANDLE_ERROR(cudaMemcpyAsync(d_position_i, position_i, sizeof(double) * (num_particle * num_dimensions), cudaMemcpyHostToDevice, stream[j]));
					HANDLE_ERROR(cudaMemcpyAsync(d_velocity_i, velocity_i, sizeof(double) * (num_particle * num_dimensions), cudaMemcpyHostToDevice, stream[j]));
					HANDLE_ERROR(cudaMemcpyAsync(d_pos_best_i, pos_best_i, sizeof(double) * (num_particle * num_dimensions), cudaMemcpyHostToDevice, stream[j]));

					HANDLE_ERROR(cudaMemcpyAsync(d_pos_best_s, pos_best_s, sizeof(double) * num_dimensions, cudaMemcpyHostToDevice, stream[j]));
					HANDLE_ERROR(cudaMemcpyAsync(d_err_best_s, err_best_s, sizeof(double), cudaMemcpyHostToDevice, stream[j]));

					cudaMemcpyAsync(BOUNDS, h_BOUNDS, sizeof(double) * num_dimensions * num_dimensions, cudaMemcpyHostToDevice, stream[j]);
					cudaMemcpyAsync(initial, h_initial, sizeof(double) * num_dimensions, cudaMemcpyHostToDevice, stream[j]);

					init_particle<<<BLOCKS, THREAD_PER_BLOCK, 0, stream[j]>>>(d_position_i, d_velocity_i, initial, numbersRand);
				}
				pso<<<BLOCKS, THREAD_PER_BLOCK, 0, stream[j]>>>(d_position_i, d_velocity_i, d_pos_best_i, d_err_best_i, d_err_i, d_pos_best_s, d_err_best_s, BOUNDS, numbersRand);

				HANDLE_ERROR(cudaMemcpyAsync(h_pos_best_s, d_pos_best_s, sizeof(double) * num_dimensions, cudaMemcpyDeviceToHost, stream[j]));
				HANDLE_ERROR(cudaMemcpyAsync(&h_err_best_s, d_err_best_s, sizeof(double), cudaMemcpyDeviceToHost, stream[j]));
